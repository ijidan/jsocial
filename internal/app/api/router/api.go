...
import (
	...
	"github.com/ijidan/jsocial/internal/app/api/controller"
...
func NewGin(conf config.App) *gin.Engine {
	var once sync.Once
	...
	once.Do(func() {
		...
		if v, ok := binding.Validator.Engine().(*validator.Validate); ok {
			...
			if err := v.RegisterValidation("image", validate.ImageValidator); err != nil {
				panic(err)
			}
		}
