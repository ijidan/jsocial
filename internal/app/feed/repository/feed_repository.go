...
func ConvertFeedToProtoFeed(feed model2.Feed) *proto_build.Feed {
	protoFeed := &proto_build.Feed{
		...
		IsEnable:     repository.EnableDbToProto[feed.IsEnable],
		ReviewStatus: repository.ReviewStatusDbToProto[feed.ReviewStatus],
		CreatedAt:    nil,
		UpdatedAt:    nil,
		DeletedAt:    nil,
	}
...
func CreateTextFeed(db *gorm.DB, userId uint64, content string) (*proto_build.Feed, error) {
	feed := &model2.Feed{
		...
		Content:      content,
		Type:         FeedTypeTxt,
		IsEnable:     repository.Enable,
		ReviewStatus: repository.ReviewPass,
	}
...
func CreateImageFeed(db *gorm.DB, userId uint64, content string, resource []string) (*proto_build.Feed, error) {
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		var feedImageList []*model2.FeedImage
		if len(resource) > 0 {
			for _, v := range resource {
				feedImage := &model2.FeedImage{
					FeedID:    feed.ID,
					ImgURL:    v,
					CreatedAt: time.Time{},
					UpdatedAt: time.Time{},
...
func CreateVideoFeed(db *gorm.DB, userId uint64, content string, resource []string) (*proto_build.Feed, error) {
	...
	err := db.Transaction(func(tx *gorm.DB) error {
		...
		var feedVideoList []*model2.FeedVideo
		if len(resource) > 0 {
			for _, v := range resource {
				feedVideo := &model2.FeedVideo{
					FeedID:    feed.ID,
					VideoURL:  v,
					CreatedAt: time.Time{},
					UpdatedAt: time.Time{},
